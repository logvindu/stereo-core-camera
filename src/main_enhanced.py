#!/usr/bin/env python3
"""
Enhanced main entry point for the Stereo Core Camera System.
Uses the complete workflow implementation.
"""
...
from src.ui import EnhancedMainWindow
from src.utils import setup_logging, create_directories, get_system_info, validate_camera_hardware


class EnhancedStereoCoreCameraApp:
    """
    Enhanced main application class for the Stereo Core Camera System.
    Implements the complete operator workflow.
    """
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.info("=== Enhanced Stereo Core Camera System Starting ===")
    ...
    def create_qt_application(self) -> bool:
        ...
        try:
            ...
            self.app.setApplicationName("Enhanced Stereo Core Camera System")
    ...
    def create_main_window(self) -> bool:
        """
        Create and show the enhanced main application window.
        Returns: True if successful, False otherwise
        """
        try:
            self.main_window = EnhancedMainWindow(self.config, self.camera, self.storage)
            self.main_window.show()
            
            self.logger.info("Enhanced main window created and shown")
            return True
            
        except Exception as e:
            self.logger.error(f"Failed to create enhanced main window: {e}")
            return False
    ...
    def run(self) -> int:
        ...
        try:
            ...
            self.logger.info("Enhanced application started successfully")
            
            # Show startup message
            if self.main_window:
                self.main_window._log_status("Enhanced Stereo Core Camera System ready")
                self.main_window._log_status("Complete workflow with image review and USB backup")
    ...
    def cleanup(self):
        ...
        try:
            self.logger.info("Cleaning up enhanced application resources...")
            ...
            if self.main_window:
                ...
                self.logger.info("Enhanced main window closed")
            ...
            self.logger.info("=== Enhanced Stereo Core Camera System Shutdown ===")
...
def main():
    ...
    app = EnhancedStereoCoreCameraApp()
    return app.run()
